...
export default function AdminDashboard() {
  ...
  const loadDocuments = async (showLoader = false) => {
  ...
  /* ================= LIVE DOCUMENT POLLING ================= */

  useEffect(() => {
    const interval = setInterval(() => {
      loadDocuments(false); // 👈 ONLY documents (no flicker)
    }, 3000);
  ...
  return (
    <div className="admin-dashboard">
      ...
      {stats && (
        <div className="stats-grid">
          <StatCard title="Total Documents" value={stats.totalDocuments} trend="+12%" />
          <StatCard title="Active Users" value={5} trend="+8%" />
          <StatCard title="Queries Today" value={40} trend="+23%" />
          <StatCard title="Avg Response" value={stats.avgResponse} trend="-15%" negative />
        </div>
      )}
      ...
      <div className="dashboard-grid">
                ...
                onChange={async (e) => {
                  ...
                  try {
                    ...
                    loadCharts();
                      ...
                      if (err === "Document already exists") {
                        alert("⚠️ This document is already uploaded");
                      } else {
                        alert("❌ Upload failed");
                      }
        ...
        <div className="card">
          <h2>Top Queries</h2>
          <Query label="How to request time off?" value={50} tag="HR" />
          <Query label="Password reset process" value={40} tag="IT" />
          <Query label="Expense reimbursement policy" value={15} tag="Finance" />
          <Query label="Remote work guidelines" value={13} tag="HR" />
          <Query label="Health insurance details" value={80} tag="Benefits" />
        </div>
      </div>
          ...
          <div className="card">
            ...
            <ResponsiveContainer width="100%" height={560}>
              <LineChart data={charts.queriesOverTime}>
                ...
                <YAxis />
                <Tooltip />
                <Line dataKey="queries" stroke="#2563eb" strokeWidth={5} />
              </LineChart>
            </ResponsiveContainer>
          </div>
